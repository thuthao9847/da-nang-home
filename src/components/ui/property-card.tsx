    ...
    <div className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <div className="p-4">
        ...
        <Link href={href} className="block">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 line-clamp-1 hover:text-[#F4A261] transition-colors">
            {title}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          <div className="text-[#F4A261] text-lg font-bold">
            ${price.toLocaleString()}
            {isRental && <span className="text-sm font-normal"> {t(`${translationPrefix}.pricePerMonth`)}</span>}
          </div>
          <Link
            href={href}
            className="px-4 py-2 bg-[#F4A261] text-white text-sm font-medium rounded-lg hover:bg-[#E76F51] transition-colors"
          >
