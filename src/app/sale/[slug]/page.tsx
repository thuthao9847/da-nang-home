...
import { getSalePropertyBySlug, saleProperties } from '@/data/properties'
import { MapPin, Bed, Bath, Square, Car, Calendar, CheckCircle } from 'lucide-react'

interface PageProps {
  params: { slug: string }
}

export default function SalePropertyDetail({ params }: PageProps) {
  const { t } = useTranslations()
  const property = getSalePropertyBySlug(params.slug)
  if (!property) return notFound()

  const similar = saleProperties.filter((p) => p.slug !== property.slug).slice(0, 3)
    ...
    <div className="py-10 bg-gray-50">
      <Container>
        <nav className="text-sm text-secondary-500 mb-6">
          ...
          <Link href="/sale" className="hover:text-primary-500">{t('navigation.sale')}</Link>
          <span className="mx-2">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="space-y-6">
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              <div className="text-primary-500 text-2xl font-bold mb-4">${property.price.toLocaleString()}</div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="flex items-center gap-2 text-secondary-700"><Car className="w-4 h-4" /> {property.garage ?? 0} {t('detail.common.garage')}</div>
            ...
            {similar.length > 0 && (
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  {similar.map((sp) => (
                    <Link key={sp.slug} href={`/sale/${sp.slug}`} className="flex gap-3 group">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-secondary-600">${sp.price.toLocaleString()}</div>
                      </div>
                    </Link>
                  ))}
                </div>
